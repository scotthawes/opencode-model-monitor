...
// v1/models liveness cross-check (v0.12.0, #90).
//
// GETs https://opencode.ai/zen/go/v1/models with the same Bearer auth as
// usage.js, on the same 30min cadence as api.json. IDs only — no pricing is
// ever derived from this endpoint. Two jobs:
//
...
//   2. Outage-vs-quota: when BOTH liveness and usage fail the provider is
//      likely down ("provider outage suspected"); a usage-only failure is a
//      quota/auth issue, not an outage. See classifyOutage().
//
...
// id array); `usageStatus`/`usageError` feed the outage-vs-quota signal but
// this function never emits the outage alert itself — monitor.js does that so
// the combined signal is reported exactly once. Never throws.
async function runLivenessWatch(stateDir, authJsonPath, pricedModels, usageStatus, usageError) {
